...
func init() {
	commands.RegisterCommand("setupowner", SetupOwner)
}

func SetupOwner(b *bot.Bot, s *discordgo.Session, m *discordgo.MessageCreate, args []string) {
	// This command can only be used by the bot owner (you)
	botOwnerID := os.Getenv("BOT_OWNER_ID")
	if botOwnerID == "" {
		botOwnerID = "174176306865504257" // Default to your ID
	}
	...
	if len(args) < 3 {
		s.ChannelMessageSend(m.ChannelID, "Usage: .setupowner <guild_id> <@user>")
		return
	}
